	...
	public static void main(String[] args) {
		try {
			...
			PrintStream benchStream = new PrintStream(new FileOutputStream(file
					+ File.separator + "find_bechnmark.txt"));

			Benchmark.Params params = new Benchmark.Params();
			params.setNumberMeasurements(50);
			benchStream.print("EMF: ");
			benchStream.println(new Benchmark(new EMFFindEval(new EMFLoadEval(
					false).call()), params));
			benchStream.flush();
			...
			// handle greql optimization
			Graph g = new JGraLabLoadEval(false).call();
			new JGraLabFindEval(g, true).call();
			benchStream.println(new Benchmark(new JGraLabFindEval(g, true),
					params));
			benchStream.flush();
			benchStream.print("JGraLab (w/o optimization): ");
			benchStream.println(new Benchmark(new JGraLabFindEval(g, false),
					params));
			benchStream.flush();
			benchStream.print("JGraLab view: ");
			// handle greql optimization
			g = new JGraLabLoadEval(true).call();
			new JGraLabFindEval(g, true).call();
			benchStream.println(new Benchmark(new JGraLabFindEval(g, true),
					params));
			benchStream.flush();
			benchStream.print("JGraLab view (w/o optimization): ");
			benchStream.println(new Benchmark(new JGraLabFindEval(g, false),
					params));
			benchStream.flush();
			benchStream.close();
