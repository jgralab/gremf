 ...
 */
package de.uni_koblenz.gremfeval.code.traverse;
...
public class BenchmarkTraverse {
		...
		try {
			...
			PrintStream logStream = new PrintStream(new FileOutputStream(
					System.getProperty("user.dir") + File.separator + "evalit"
							+ File.separator + ".traverse_log.txt"));
			...
			PrintStream benchStream = new PrintStream(new FileOutputStream(file
					+ File.separator + "traverse_bechnmark.txt"));

			Benchmark.Params params = new Benchmark.Params();
			params.setNumberMeasurements(50);
			benchStream.print("EMF: ");
			benchStream.println(new Benchmark(new EMFTraverseEval(
					new EMFLoadEval(false).call()), params));
			benchStream.flush();
			benchStream.print("EMF view: ");
			benchStream.println(new Benchmark(new EMFTraverseEval(
					new EMFLoadEval(true).call()), params));
			benchStream.flush();
			benchStream.print("JGraLab: ");
			benchStream.println(new Benchmark(new JGraLabTraverseEval(
					new JGraLabLoadEval(false).call()), params));
			benchStream.flush();
			benchStream.print("JGraLab view: ");
			benchStream.println(new Benchmark(new JGraLabTraverseEval(
					new JGraLabLoadEval(true).call()), params));
			benchStream.flush();
			benchStream.close();
