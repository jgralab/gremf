...
public class JGraLabObjectHandlingEval implements Runnable {

	private final boolean withGrEMF;

	public JGraLabObjectHandlingEval(boolean withGrEMF) {
	...
	public void run() {
		...
		for (PSequence<Vertex> tuple : vertices) {
			g.createVertex(tuple.get(0).getAttributedElementClass());
		}
		Vertex vertexB = g.createVertex(vertexA.getAttributedElementClass());
		vertexB.setAttribute("name", "foo");
