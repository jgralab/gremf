...
public class EMFObjectHandlingWithProxyEval implements Runnable {
	...
	public void run() {
		...
		EObject graph = objA.eResource().getContents().get(0);
		List<EObject> objs = (List<EObject>) graph.eGet(graph.eClass()
				.getEStructuralFeature("vertices"));
		objs.removeAll(objects.keySet());
	}
}
