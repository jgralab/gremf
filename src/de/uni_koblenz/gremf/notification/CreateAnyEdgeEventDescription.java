...
public class CreateAnyEdgeEventDescription extends CreateEdgeEventDescription {

	public CreateAnyEdgeEventDescription(
			de.uni_koblenz.jgralab.eca.events.EventDescription.EventTime time) {
	...
	public void fire(EdgeClass type) {
		...
		Graph graph = this.getActiveECARules().get(0).getECARuleManager()
				.getGraph();
		for (ECARule<EdgeClass> rule : this.activeRules) {
			rule.trigger(new CreateEdgeEvent(nested, graph, this.getType()));
		}
	}
}
